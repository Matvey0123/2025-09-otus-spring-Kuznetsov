...
class TestServiceImplTest {
    ...
    void shouldGetTestResultUponNoErrors() {
        ...
        inOrder.verify(ioService).readIntForRangeWithPrompt(1, 2, question1AsString,
                "Incorrect answer's number! Please, try again");
        inOrder.verify(questionConverter).convertQuestionToString(secondQuestion);
        inOrder.verify(ioService).readIntForRangeWithPrompt(1, 2, question2AsString,
                "Incorrect answer's number! Please, try again");
    }
    ...
    void shouldGetWrongAnswerUponExceptionDuringGetAnswer() {
        ...
        verify(ioService).printLine("Attempts to answer the question have expired. Answer is considered to be wrong");
    }
